...
export const ToolPart = memo(
  ({
    ...
    if (toolCall.toolName === "editNote") {
      return <EditNoteToolCall toolCall={toolCall} />
    }
...
const EditNoteToolCall = memo(({ toolCall }: { toolCall: ToolCallDisplay }) => {
  const [isExpanded, setIsExpanded] = useState(false)
  const outputObj =
